...
server <- function(input, output, session) {
  ...
  pitcher_data <- eventReactive(input$submit, {
    ...
    Pitcher_ID <- chadwick %>% dplyr::filter(mlb_played_last == 2025, name_last == input$last_name, name_first == input$first_name) %>% dplyr::pull(key_mlbam)
    #Pitcher_ID <- Pitcher_id %>% pull(mlbam_id)
    baseballr::statcast_search_pitchers(start_date = "2025-03-28", end_date = "2025-12-01", pitcherid = Pitcher_ID)
  })
