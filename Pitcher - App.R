...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      textInput("first_name", "First Name", value = "Hunter"),
      textInput("last_name", "Last Name", value = "Greene"),
      actionButton("submit", "Get Player Data (Wait 10-15 Seconds)"),
      br(),
  ...
  output$releaseHeightPlot <- renderPlotly({
    ...
    plot_list <- lapply(pitch_types, function(pitch) {
      ...
      gg <- ggplot(pitch_data, aes(x = release_pos_x, y = release_pos_z)) + 
        geom_point() + 
        labs(title = paste("Release Position of", pitch, "Pitches"), x = "Release Position X (feet)", y = "Release Position Z (feet)") + 
        theme_minimal()
